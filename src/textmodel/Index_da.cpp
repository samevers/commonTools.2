...
int32_t WhiteBlackList_::MakeIndex(const char *file, const char *index_file)
{
	...
	*/	
		...
		if(len>=MAXSTRLEN/2 || len < 0){continue;}
		buffer[len*2] = '\0';
		string gQuery(buffer);
		cerr << "[debug_sam] gQuery = "<< gQuery<<  endl;
		node.query = gQuery;					// 赋值给结构体, node.query
		Pos_Reform option_node(vrtype.c_str(),vrlabel.c_str(),  type.c_str(),reform.c_str(), item_query.c_str(), data.c_str(), iswap_pc.c_str(), q_len);
		//reform_size += reform.length();
...
{
	//cerr << "[debug_sam] content = " << content << endl;
...
// change to quanjiao, and normalize to lower case.
...
/*************************************** For 2 column dict file.******************************************************/
// make index
// Format of inputfile: 2 columns
...
/*************************************** For 1 column dict file.******************************************************/
// make index
// Format of inputfile: 2 columns
int32_t WhiteBlackList_::MakeIndex_1column( const char *file, const char *index_file, Darts::DoubleArray& da_)
{
	FILE * fp_in = fopen(file, "r");
	if (!fp_in) 
	{
	...
	while (fgets(line, 10240, fp_in)) 
	{
		...
		string gQuery(buffer);
		delete [] buffer;
		vect.push_back(gQuery);
		
	}
	//将数据进行转换并输出到index文件中
	OutputIndexFile_1column(vect, index_file, da_);	//	将要进行DA存储的vector输出到索引文件 index_file
...
//输出index文件
int32_t WhiteBlackList_::OutputIndexFile_1column(std::vector<string> & vect,const char * index_file, Darts::DoubleArray& da_) 
{
	...
	if(da_.build(da_key_vect.size(), &da_key_vect[0], 0, &da_value_vect[0]) != 0)
	{
		fprintf(stderr, "build da error!\n");
		return -1;
	}
	...
	// 写入da size
	uint32_t da_size =
		da_.size() * da_.unit_size();
	fwrite(&da_size, sizeof(da_size), 1, fp_index);
	fclose(fp_index);
	if (da_.save(index_file, "ab") != 0)
	{
		fprintf(stderr, "save index error!\n");
		return -1;
	}
...
int32_t WhiteBlackList_::LoadIndex_1column(const char *index_file, 
									Darts::DoubleArray* &da_)
{
	FILE *fp_in = fopen(index_file, "rb");
	uint32_t index_file_size = 0;
